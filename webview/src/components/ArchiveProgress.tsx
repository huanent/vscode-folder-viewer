		...
		<section className="fixed right-4 bottom-4 z-15 flex min-h-21.5 w-[min(440px,calc(100%-32px))] items-center gap-3.5 rounded-md border border-widget-border bg-notification px-3.5 py-3 shadow-widget" role="status" aria-live="polite">
			<div className="min-w-0 flex-1">
				<div className="flex justify-between gap-3 font-semibold">
					<span>{operation.kind === 'compress' ? 'Compressing' : 'Extracting'}</span>
			...
			<button type="button" disabled={operation.cancelling} className="h-7 min-w-16 shrink-0 cursor-pointer rounded-sm border-0 bg-button px-3 text-button-foreground hover:not-disabled:bg-button-hover disabled:cursor-default disabled:opacity-60" onClick={actions.cancelArchive}>
				{operation.cancelling ? 'Stopping...' : 'Stop'}
			</button>
		</section>
	);
}