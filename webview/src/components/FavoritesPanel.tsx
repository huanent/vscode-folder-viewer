...
export function FavoritesPanel({ state, actions }: FavoritesPanelProps) {
	return (
		<section
			className="absolute top-[calc(100%+4px)] left-0 z-10 max-h-[min(420px,calc(100vh-64px))] w-full min-w-64 overflow-y-auto rounded border border-widget-border bg-menu p-1.5 shadow-widget"
			aria-label="Favorites"
			onClick={event => event.stopPropagation()}
			onMouseDown={event => event.preventDefault()}
		>
			<nav className="mb-1 grid grid-cols-2 border-b border-menu-separator pb-1 min-[600px]:grid-cols-4" aria-label="Quick locations">
				{quickLocations.map(({ location, icon, label }) => (
					<button
						key={location}
						type="button"
						title={label}
						className="flex h-8 min-w-0 cursor-pointer items-center justify-center gap-1.5 rounded-sm border-0 bg-transparent px-1.5 text-menu-foreground hover:bg-menu-selection hover:text-menu-selection-foreground focus-visible:outline focus-visible:-outline-offset-1 focus-visible:outline-focus"
						onClick={() => {
							actions.setPathInputOpen(false);
							actions.navigateQuickLocation(location);
						}}
					>
						<i className={`codicon ${icon} shrink-0 text-base text-icon`} aria-hidden="true" />
			...
			{state.favoriteUris.length === 0 ? (
				...
				return (
					<div
						key={uri}
						className="group my-px flex h-8 w-full min-w-0 items-center rounded-sm text-menu-foreground hover:bg-menu-selection hover:text-menu-selection-foreground"
					>
						<button
							type="button"
							title={relativePath}
							className="flex h-full min-w-0 flex-1 cursor-pointer items-center gap-2 overflow-hidden border-0 bg-transparent px-2 text-left text-inherit focus:outline focus-visible:-outline-offset-1 focus-visible:outline-focus"
							onClick={() => {
								actions.setPathInputOpen(false);
						...
						<IconButton
							icon="codicon-trash"
							className="mr-0.5 shrink-0 text-menu-foreground opacity-0 hover:bg-transparent group-hover:opacity-100 group-focus-within:opacity-100"
